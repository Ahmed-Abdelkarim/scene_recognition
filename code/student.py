...
def get_tiny_images(image_paths):
    ...
    """
    img_array = np.zeros((len(image_paths), 16*16))
    # TODO: Implement this function!
    for i, path in enumerate(image_paths):
       img_array[i] = resize(imread(path, as_gray = True), (16, 16), anti_aliasing=True).flatten()
       img_array[i] = img_array[i]/ np.linalg.norm(img_array[i])
    return img_array
